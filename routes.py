...
def register_routes(api, get_db_session_func):
    ...
    def create_broadcast():
        ...
        try:
            ...
            if latest_time <= earliest_time:
                return jsonify({'error': 'latestTime must be after earliestTime'}), 400
            ...
            db_session.rollback()
            return jsonify({'error': f'Failed to create broadcast: {str(e)}'}), 400
    
    @api.route('/broadcasts/nearby', methods=['GET'])
    def find_nearby_broadcasts():
        ...
        try:
            ...
            if latitude is None:
                return jsonify({'error': 'Missing required parameter: latitude'}), 400
            if longitude is None:
                return jsonify({'error': 'Missing required parameter: longitude'}), 400
            if radius is None:
                return jsonify({'error': 'Missing required parameter: radius'}), 400
            ...
            return jsonify(nearby_broadcasts), 200
            
        except Exception as e:
            return jsonify({'error': f'Failed to find nearby broadcasts: {str(e)}'}), 400
    
    @api.route('/broadcasts/<string:broadcast_id>', methods=['PUT'])
        ...
        try:
            ...
            if 'earliestTime' in data:
                try:
                    ...
                    return jsonify({'error': f'Invalid earliestTime format: {str(e)}'}), 400
            ...
            if 'latestTime' in data:
                try:
                    ...
                    return jsonify({'error': f'Invalid latestTime format: {str(e)}'}), 400
            ...
            db_session.rollback()
            return jsonify({'error': f'Failed to update broadcast: {str(e)}'}), 400
    
    @api.route('/broadcasts/<string:broadcast_id>', methods=['DELETE'])
    def delete_broadcast(broadcast_id):
        ...
        try:
            ...
            db_session.rollback()
            return jsonify({'error': f'Failed to delete broadcast: {str(e)}'}), 400
